...
class GraphQLAuthzDiffSmokeTest < Minitest::Test
  ...
  def test_graphql_authz_diff_flags_unexpected_access
    ...
    Dir.mktmpdir('graphql-authz-diff-smoke-') do |tmp_dir|
      ...
      assert(report[:opaque_handle_family_count] >= 1)
      assert(report[:opaque_handle_reportable_count] >= 1)
      assert_equal('direct_denied_alternate_accessible', report.dig(:opaque_handle_atlas, :best_candidate, :report_angle))
      ...
      refute_nil(report.dig(:lifecycle_summary, :submission_bundle, :best_object_family_candidate, :family_key))
      ...
      atlas_json = File.join(tmp_dir, 'graphql-authz-diff-smoke', 'opaque_handle_atlas.json')
      atlas_markdown = File.join(tmp_dir, 'graphql-authz-diff-smoke', 'opaque_handle_atlas.md')
      ...
      assert(File.exist?(atlas_json))
      assert(File.exist?(atlas_markdown))
